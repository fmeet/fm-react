    ...
    new Command({
        ...
        runFn: function(roomId, args) {
            if (args) {
                ...
                //   /join https://link.facemeeting.com/#/!example:example.org?via=altserver.com
                // The command also supports event permalinks transparently:
                //   /join https://link.facemeeting.com/#/!example:example.org/$something:example.org
                //   /join https://link.facemeeting.com/#/!example:example.org/$something:example.org?via=altserver.com
                ...
                if (params[0].startsWith("http:") || params[0].startsWith("https:")) {
                    ...
                    // if we're using a FaceMeeting permalink handler, this will catch it before we get much further.
            ...
            return success((async () => {
                if (isPhoneNumber) {
                    ...
                    if (!results || results.length === 0 || !results[0].userid) {
                        throw new Error("Unable to find FM ID for phone number");
                    }
