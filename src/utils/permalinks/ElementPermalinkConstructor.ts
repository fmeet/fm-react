/*
Copyright 2019, 2021 FM Foundation
...
export default class ElementPermalinkConstructor extends PermalinkConstructor {
    ...
    constructor(elementUrl: string) {
        ...
        if (!this.elementUrl.startsWith("http:") && !this.elementUrl.startsWith("https:")) {
            throw new Error("FaceMeeting prefix URL does not appear to be an HTTP(S) URL");
        }
    }
    ...
    // Adapted for FaceMeeting's URL format
    ...
    /**
     * Parses an app route (`(user|room|group)/identifer`) to a FM entity
     * (room, user, group).
